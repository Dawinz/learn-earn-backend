 ...
 */
export async function recordEarning(req: AuthenticatedRequest, res: Response) {
  try {
    const { deviceId } = req;
    ...
    const coins = calculateCoinRewards(
      source as any,
      settings.eCPM_USD,
      settings.coinToUsdRate
    );
    ...
    const capCheck = checkDailyEarningCap(totalEarningsToday, settings.maxDailyEarnUsd);
 ...
 */
export async function getDailyEarnings(req: AuthenticatedRequest, res: Response) {
  try {
    const { deviceId } = req;
