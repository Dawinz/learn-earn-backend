...
async function requestPayout(req, res) {
    try {
        ...
        const amountValidation = (0, economics_1.validatePayoutAmount)(amountUsd, settings.minPayoutUsd);
        ...
        // Check cooldown
        const cooldown = await Cooldown_1.default.findOne({ deviceId });
        ...
        await Cooldown_1.default.findOneAndUpdate({ deviceId }, {
            nextPayoutAt,
            reason: 'cooldown'
        }, { upsert: true });
...
async function getPayoutStatus(req, res) {
    try {
        ...
        const payout = await Payout_1.default.findOne({
            _id: payoutId,
            deviceId
        });
        if (!payout) {
